...
clock_t matmultCUDA(const float* a, int lda, const float* b, int ldb, float* c, int ldc, int n, int NUM_THREADS);
__global__ static void matMultCUDA(const float* a, size_t lda, const float* b, size_t ldb, float* c, size_t ldc, int n);

int main(int argc, char* argv[]) {
    if (argc < 3) {
        printf("Usage: program_name NUM_THREADS N\n");
        return 0;
    }
    ...
    clock_t time1 = matmultCUDA(a, N, b, N, c, N, N, NUM_THREADS);
    ...
    printf("Time used: %.2f seconds(%.2lf GFLOPS) in CUDA, Time used: %.2f seconds(%.2lf GFLOPS) in CPU\n", sec1, 2.0 * N * N * N / (sec1 * 1E9), sec2, 2.0 * N * N * N / (sec2 * 1E9));
